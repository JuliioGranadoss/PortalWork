<ul
	class="navbar-nav bg-gradient-dark sidebar sidebar-dark accordion toggled" id="accordionSidebar">
    ...
    <a class="sidebar-brand d-flex align-items-center justify-content-center" href="{{ route('welcome') }}">
		<div class="sidebar-brand-icon">
			...
			@if ($url)
                <img
                width="70px" class="img-profile" src="{{ $url }}">
            @else
                <i class="fas fa-laugh-wink"></i>
            @endif
		</div>
		<div class="sidebar-brand-text mx-3">{{ __('PortalWork') }}</div>
	</a>
	...
	@role('admin|worker')
	<!-- Heading --><div
		class="sidebar-heading"> {{__('Bolsa de trabajo')}}
	</div>
	...
	<li class="nav-item">
		<a class="nav-link" href="{{ route('offers.index') }}">
			...
			<span>{{__('Ofertas de Trabajo')}}</span>
		</a>
	</li>
	@endrole
	...
	@role('admin|manager')
	<!-- Heading --><div
		class="sidebar-heading"> {{__('Inventario')}}
	</div>
	...
	<!-- Divider -->
	<hr
	class="sidebar-divider"> <!-- Heading -->
	<div
		class="sidebar-heading">{{__('Ajustes')}}
	</div>
	...
	<!-- Divider -->
	<hr
	class="sidebar-divider d-none d-md-block"> <!-- Sidebar Toggler (Sidebar) -->
